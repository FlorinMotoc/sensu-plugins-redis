...
require_relative '../../bin/check-redis-info-thresholds.rb'

describe 'RedisInfoThresholdsCheck', '#run' do
  before(:all) do
    RedisInfoThresholdsCheck.class_variable_set(:@@autorun, nil)
  end

  it 'accepts config' do
    args = %w[--host 10.0.0.1 --port 3456 --password foobar]
    check = RedisInfoThresholdsCheck.new(args)
